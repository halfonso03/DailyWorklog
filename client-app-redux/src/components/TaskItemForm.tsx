...
import { addItem } from '../redux/logSlice';
...
const TaskItemForm = ({ taskItem, onAdded, defaultDate }: Props) => {
  ...
  // const { selectedTask } = useLogContext();
  ...
  const { projects, hidtas, requestors } = useAppSelector((state) => state.log);
  ...
  async function handleLoadRequestors(hidtaId: number) {
    fetchRequestors(hidtaId, dispatch);
  }
  ...
  useEffect(() => {
    if (taskItem) {
      fetchRequestors(taskItem.hidtaId, dispatch);
    }
  }, [dispatch, taskItem]);
          ...
          onSubmit={async (values, { setSubmitting }) => {
            try {
              ...
              if (values.id !== 0) {
                ...
                await dispatch(addItem(JSON.stringify(values)));
